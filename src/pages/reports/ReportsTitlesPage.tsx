...
export default function ReportsTitlesPage(){ const now=new Date(); const [deviceId,setDeviceId]=useState(''); const [appName,setAppName]=useState(''); const [from,setFrom]=useState(toLocalInputValue(new Date(now.getTime()-24*60*60*1000))); const [to,setTo]=useState(toLocalInputValue(now)); const [top,setTop]=useState(20);
const devicesQ=useQuery({queryKey:['devices'],queryFn:listDevices}); const options=(devicesQ.data??[] as any[]).map(d=>({ value:d.deviceId, label:`${d.deviceId}${d.username?' — @'+d.username:''}`, username:d.username }));
