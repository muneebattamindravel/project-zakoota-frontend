...
export default function ReportsAppsPage(){ const now=new Date(); const [deviceId,setDeviceId]=useState(''); const [from,setFrom]=useState(toLocalInputValue(new Date(now.getTime()-24*60*60*1000))); const [to,setTo]=useState(toLocalInputValue(now)); const [top,setTop]=useState(20);
