...
export default function LoginPage() {
  const { login, isAuthed, bootstrapped } = useAuth();
  ...
  // If already logged in and we somehow land on /login, bounce to /devices
  useEffect(() => {
    if (!bootstrapped) return;
    if (isAuthed) {
      nav("/devices", { replace: true });
    }
  }, [bootstrapped, isAuthed, nav]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      push({
        tone: "success",
        title: "Logged in",
        desc: "Welcome back to matrixFlow.",
      });

      // ✅ Redirect to previous page or devices dashboard
