...
export default function DeviceCard({
  ...
  const workStartLabel = firstChunkAt
  ...
  const lastPending =
    summary?.lastPending ?? device?.commandsSummary?.lastPending ?? null;
  const lastAck =
    summary?.lastAck ?? device?.commandsSummary?.lastAck ?? null;
                  ...
                  style={{
                    width:
                      activity.total > 0
                        ? `${Math.min(
                            100,
                            (activity.active / Math.max(activity.total, 1)) *
                              100
                          )}%`
                        : "0%",
                  }}
                  ...
                  style={{
                    width:
                      activity.total > 0
                        ? `${Math.min(
                            100,
                            (activity.idle / Math.max(activity.total, 1)) *
                              100
                          )}%`
                        : "0%",
                  }}
            ...
            {lastPending && (
              <div>
                ...
                <span className="text-slate-400">
                  ({fmtLocal(lastPending.createdAt)})
                </span>
              </div>
            )}
            {lastAck && (
              <div>
                ...
                <span className="text-slate-400">
                  ({fmtLocal(lastAck.acknowledgedAt)})
                </span>
              </div>
            )}
          </div>
