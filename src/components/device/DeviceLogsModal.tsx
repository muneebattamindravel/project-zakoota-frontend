      ...
      <div className="rounded-xl border bg-white overflow-hidden">
            ...
            <tbody>
              {q.isLoading ? (
                ...
                items.map((row) => {
                  ...
                  return (
                    <tr key={String(_id ?? `${startAt}-${endAt}-${Math.random()}`)} className="border-b last:border-0">
                      <td className="px-3 py-2">
                        <div className="text-slate-900">{fmtDate(startAt)} {fmtTime12(startAt)}</div>
                        <div className="text-[11px] text-slate-500">{fmtLocal(startAt)}</div>
                      </td>
                      <td className="px-3 py-2">
                        <div className="text-slate-900">{fmtDate(endAt)} {fmtTime12(endAt)}</div>
                        <div className="text-[11px] text-slate-500">{fmtLocal(endAt)}</div>
                      </td>
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t bg-slate-50">
          ...
          <button
            ...
            disabled={skip + limit >= total || q.isFetching}
          >
            Next
          </button>
        </div>
