...
import { getDeviceTitles } from "../../utils/api";
import type { TitlesListResponse } from "../../utils/types";
...
export default function DeviceTitlesModal({ open, onClose, device }: Props) {
  ...
  const q = useQuery<TitlesListResponse>({
    enabled: open && !!device?.deviceId,
    queryKey: ["device-titles", device?.deviceId, from, to, skip, limit],
    queryFn: () => getDeviceTitles({ deviceId: device.deviceId, from, to, skip, limit }),
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Titles · ${device?.name || device?.username || "Unassigned"} — ${device?.deviceId || ""}`} className="max-w-4xl">
      {/* Filters */}
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-slate-600 border-b">
              <tr>
                ...
                <th className="text-left font-medium px-3 py-2">Occurrences</th>
              </tr>
            </thead>
            <tbody>
              {q.isLoading ? (
                <tr><td colSpan={4} className="px-3 py-6"><div className="flex items-center gap-2 text-slate-600"><Spinner /> Loading titles…</div></td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={4} className="px-3 py-6 text-slate-500">No data for this range.</td></tr>
              ) : (
                rows.map((r) => {
                  const title = r.title || r.name || r._id || "—";
                  const active = r.activeSeconds ?? r.active ?? 0;
                  const idle = r.idleSeconds ?? r.idle ?? 0;
                  const occurrences = r.occurrences ?? r.count ?? 0;
                  return (
                    <tr key={String(r._id ?? title)} className="border-b last:border-0">
                      <td className="px-3 py-2">{title}</td>
                      <td className="px-3 py-2">{fmtHMS(active)}</td>
                      <td className="px-3 py-2">{fmtHMS(idle)}</td>
                      <td className="px-3 py-2">{occurrences}</td>
                    </tr>
                  );
                })
              )}
